...
import { desmosifyObjMesh, serializeForDesmos } from "./multi-obj-bvh-to-desmoscript.mjs";
import { ParsedMultiOBJ, parseMultiObj } from "./multi-obj-importer.mjs";
import * as path from "node:path";
import * as fs from "node:fs/promises";

export function multiOBJToDesmoscriptInner(namespaceName: string, obj: ParsedMultiOBJ, a: MacroAPI) {
  const expressions: ASTExpr[] = [];
  const serializedMeshes = new Map(Array.from(obj.objects.entries()).map(([name, mesh]) => [name, {
  ...
  for (const [meshName, serializedMesh] of serializedMeshes.entries()) {
    if ( meshLUTExprData.length == 0 || lastof(meshLUTExprData).length + serializedMesh.mesh.length > 10000) {
  ...
  expressions.push(
    a.binop(a.ident("xmin"), "=", a.list(...aabbs.map(aabb => a.number(aabb.min[0]))))
  );
  expressions.push(
    a.binop(a.ident("ymin"), "=", a.list(...aabbs.map(aabb => a.number(aabb.min[1]))))
  );
  expressions.push(
    a.binop(a.ident("zmin"), "=", a.list(...aabbs.map(aabb => a.number(aabb.min[2]))))
  );
  expressions.push(
    a.binop(a.ident("xmax"), "=", a.list(...aabbs.map(aabb => a.number(aabb.max[0]))))
  );
  expressions.push(
    a.binop(a.ident("ymax"), "=", a.list(...aabbs.map(aabb => a.number(aabb.max[1]))))
  );
  expressions.push(
    a.binop(a.ident("zmax"), "=", a.list(...aabbs.map(aabb => a.number(aabb.max[2]))))
  );

  expressions.push(
    a.binop(a.ident("outerIndex"), "=", a.list(...meshIndices.map(e => a.number(e.outerIndex))))
  );
  expressions.push(
    a.binop(a.ident("innerIndex"), "=", a.list(...meshIndices.map(e => a.number(e.innerIndex))))
  );
  expressions.push(
    a.binop(a.ident("size"), "=", a.list(...meshIndices.map(e => a.number(e.size))))
  );

  expressions.push(a.fromstr(`fn doLookup(testxmin, testymin, testzmin, testxmax, testymax, testzmax) {
...
export const multiObjToDesmoscript: ScopeContent.Macro["fn"] = async function (expr, ctx, a) {
...
export const lookupMesh: ScopeContent.Macro["fn"] = async function (expr, ctx, a) {
  ...
  expressions.push(a.binop(a.ident("foundMeshIndices"),
    "=",
    a.fn(
  ...
  .forEach((component, componentIndex) => {
    vertexPositionComponents.push(a.binop(a.ident(component),
      "=",
      a.fn(a.ident("join"), 
        ...new Array(meshCount).fill(0)
        .map((e, i) => {

          const meshVertexGetter = a.fn(a.ident("bin", "getComponentOfVertexOfMesh"),
          ...
          return a.match([
            [a.binop(a.fn(a.ident("length"), a.ident("foundMeshIndices")), ">=", a.number(i + 1)), meshVertexGetter]
          ], a.list());
        })
      )
    ));
