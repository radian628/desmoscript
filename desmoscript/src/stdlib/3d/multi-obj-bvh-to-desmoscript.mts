...
export function getNormal(a: [number, number, number], b: [number, number, number], c: [number, number, number]): [number, number, number] {
  return cross(
    ...
    centeredMesh.vertices.map(v => {
      return {
        position: v.position.map(c => 
          Math.min(Math.max(Math.floor(c / largestScale * POS_COMPONENT_RANGE / 2) + POS_COMPONENT_RANGE / 2, 0), 65535)
