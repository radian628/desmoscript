...
function iterateOverBitField<T extends string>(
  spec: BitFieldSpec<T>,
  callback: (
    name: T,
    about: BitFieldSpecElement,
    currentBit: number,
...
function createBitFieldGetters<T extends string>(
  ...
  iterateOverBitField(spec, (name, about, bit, num) => {
    ...
    const getDataElement = nolist
      ? a.ident("data")
      : a.binop(a.ident("data"), "[", a.number(num));
        ...
        a.fndef(
          ...
          [
            a.fn(
              ...
              a.fn(
                a.ident("floor"),
                a.binop(getDataElement, "/", a.number(2 ** bit))
              ),
