...
function findIdentifier(ident: ASTIdentifier<{ equivalentScope?: Scope, innerScope?: Scope}>): string[] {
            ...
            if (segmentValidatorScope.contents.has(ident.segments[segmentIndex])) {
                ...
                if (segmentIndex == ident.segments.length - 1) {
                    if ([
                        ...
                        return createIdentifierScopeChain(ident, searchScope);
                    }
                } else {
    ...
    function compileSingleExpression(rootExpr: ScopedASTExpr): string {
        function c(e: ScopedASTExpr): string {
            ...
            case ASTType.IDENTIFIER:
                return toDesmosVar(findIdentifier(e).join("NSSEP"));
            case ASTType.POINT:
